...
medication_classes = ["Anxiolytics", "Benzodiazepines", "Antidepressants", "Mood Stabilizers", "Antipsychotics",
                      "Stimulants"]
therapy_classes = ["TherapyA", "TherapyB", "TherapyC", "TherapyD"]  # Update with your types
...
def get_concise_rewrite(text, max_tokens, temperature=0.7):
    messages = [
        {"role": "system", "content": (
            "You are an expert rewriting assistant. Rewrite the given statement into a concise version while preserving its tone and vocabulary."
        )},
...
def get_explanation(patient_statement, predicted_diagnosis):
    messages = [
        {"role": "system", "content": (
            "You are an expert mental health assistant. Provide a concise, evidence-based explanation of how the patient's statement supports the diagnosis."
        )},
        {"role": "user", "content": (
...
def get_user_info(username):
    ...
    conn.close()
    if user:
        return f"Username: {user[0]}<br>Full Name: {user[2]}<br>Email: {user[1]}"
    else:
...
def insert_patient_session(session_data):
    # session_data is a dict with keys: username, patient_name, age, gender, symptoms, diagnosis, medication, therapy, summary, explanation, pdf_report, appointment_date (optional)
    conn = sqlite3.connect("../users.db")
    c = conn.cursor()
    c.execute("""
    ...
    session_data.get("therapy"),
    session_data.get("summary"), session_data.get("explanation"), session_data.get("pdf_report"),
...
def generate_pdf_report(session_data):
    ...
    pdf.ln(10)
    # Use multi_cell so text wraps automatically.
...
def get_previous_patient_info(selected_patient):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT patient_name, age, gender FROM patient_sessions WHERE patient_name=? ORDER BY session_timestamp DESC LIMIT 1",
        (selected_patient,))
    record = c.fetchone()
    conn.close()
    if record:
...
with gr.Blocks(theme="soft") as app:
    ...
    # ----- Page: Registration (hidden by default) -----
    ...
    with gr.Tabs(visible=False, elem_id="main_panel") as main_panel:
        with gr.Tab("Chatbot"):
            with gr.Row():
                # Left: Inputs (including previous patients dropdown)
                ...
                # Right: Outputs (HTML for colored confidence)
                with gr.Column(scale=1):
                    Diagnosis = gr.HTML(label="Diagnosis")
                    Medication = gr.HTML(label="Medication")
                    Therapy = gr.HTML(label="Therapy")
                    Summary = gr.HTML(label="Concise Summary")
                    Explanation = gr.HTML(label="Explanation")
            with gr.Row():
            ...
            def handle_chat_extended(patient_name, gender, age, symptoms):
                ...
                diagnosis_output = f"{diagnosis_pred} (<span style='color:green;'>{diagnosis_conf * 100:.1f}% confidence</span>)"
                (med_pred, med_conf), (therapy_pred, therapy_conf) = predict_med_therapy(symptoms, age, gender)
                medication_output = f"{med_pred} (<span style='color:green;'>{med_conf * 100:.1f}% confidence</span>)"
                therapy_output = f"{therapy_pred} (<span style='color:green;'>{therapy_conf * 100:.1f}% confidence</span>)"
                ...
                if user_session.value:
                    ...
                    conn.commit()
                conn.close()
                return diagnosis_output, medication_output, therapy_output, summary, explanation, gr.update(
                    visible=True)


            submit.click(handle_chat_extended,
                         inputs=[patient_name_input, gender_input, age_input, symptoms_input],
                         outputs=[Diagnosis, Medication, Therapy, Summary, Explanation, generate_report_btn])
        ...
        with gr.Tab("Chat History"):
            history_output = gr.HTML(label="Chat History")
            ...
            def load_history():
                if user_session.value:
                    ...
                    return "<br>".join([f"[{h[2]}] {h[0]}<br>Bot: {h[1]}" for h in history])
                else:
        ...
        with gr.Tab("Book an Appointment"):
            with gr.Row():
                ...
                with gr.Column():
                    booking_output = gr.HTML(label="Booking Confirmation")
            ...
            def book_appointment(patient_name, doctor_name, appointment_date, appointment_time, reason):
                ...
                confirmation = (
                    f"Appointment booked for {patient_name} with {doctor_name} on {appointment_date} at {appointment_time}.<br><br>"
                    f"<b>Reason:</b> {reason}"
                )
                return confirmation
        ...
        with gr.Tab("Patient Sessions"):
            ...
            sessions_output = gr.HTML(label="Sessions")
            ...
            def search_sessions(name, date):
                ...
                if sessions:
                    output = "<br><br>".join([
                                                 f"Patient: {s[0]}<br>Age: {s[1]}<br>Gender: {s[2]}<br>Symptoms: {s[3]}<br>Diagnosis: {s[4]}<br>Medication: {s[5]}<br>Therapy: {s[6]}<br>Summary: {s[7]}<br>Explanation: {s[8]}<br>Report: {s[9]}<br>Session Time: {s[10]}"
                                                 for s in sessions])
                    return output
                else:
    ...
    profile_button.click(toggle_profile,
                         inputs=[user_session, profile_visible],
                         outputs=[profile_info_box, profile_visible, profile_info])

    def logout_callback():
        user_session.value = ""
        profile_visible.value = False
        return "Logged out.", gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), gr.update(
            visible=False)


    logout_button.click(logout_callback,
                        outputs=[profile_info, main_panel, profile_info_box, login_page, header_row])
